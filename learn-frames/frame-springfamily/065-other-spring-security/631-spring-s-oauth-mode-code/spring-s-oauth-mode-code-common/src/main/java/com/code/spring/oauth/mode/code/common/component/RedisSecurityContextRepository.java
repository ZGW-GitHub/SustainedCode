...
public class RedisSecurityContextRepository implements SecurityContextRepository {
	...
	public void saveContext(SecurityContext context, HttpServletRequest request, HttpServletResponse response) {
		...
		if (emptyContext.equals(context)) {
			redissonClient.getBucket(SPRING_SECURITY_CONTEXT_KEY + nonce).delete();
		} else {
			// 保存认证信息
			RBucket<String> bucket = redissonClient.getBucket(SPRING_SECURITY_CONTEXT_KEY + nonce, StringCodec.INSTANCE);
			bucket.set(JSONUtil.toJsonStr(context), DEFAULT_TIMEOUT_SECONDS, TimeUnit.MILLISECONDS);
		}
	}
	...
	public boolean containsContext(HttpServletRequest request) {
		...
		// 检验当前请求是否有认证信息
		return redissonClient.getBucket(SPRING_SECURITY_CONTEXT_KEY + nonce).isExists();
	}
	...
	private SecurityContext readSecurityContextFromRedis(HttpServletRequest request) {
		...
		// 根据缓存 id 获取认证信息
		Object contextCache = redissonClient.getBucket(SPRING_SECURITY_CONTEXT_KEY + nonce).get();
		if (contextCache instanceof String contextStr) {
			return JSONUtil.toBean(contextStr, SecurityContext.class);
		}
		return null;
	}
